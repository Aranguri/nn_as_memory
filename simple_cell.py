...
class SimpleCell(tf.contrib.rnn.RNNCell):
    def __init__(self, output_size, batch_size, h_size, memory_length, memory_size, shift_length):
        ...
        self.c2o_w = tf.Variable(tf.truncated_normal((h_size, output_size), stddev=1.0 / np.sqrt(h_size + memory_size)))#change output_size --> h_size + memory_size
    ...
    def __call__(self, x, prev_state):
        ...
        pop = tf.Print([0], [self.c2o_w[1][1]])
        with tf.control_dependencies([pop]):
            w_list = tf.concat((w_read, w_write), axis=0)
        return out, NTMState(ctrl_state=ctrl_state, read_list=read_list, w_list=w_list, memory=memory)
